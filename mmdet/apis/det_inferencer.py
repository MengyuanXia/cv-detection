...
class DetInferencer(BaseInferencer):
    ...
    def _load_weights_to_model(self, model: nn.Module,
        ...
        if checkpoint is not None:
            ...
            #     }
            if 'dataset_meta' in checkpoint_meta:
                if isinstance(checkpoint_meta['dataset_meta'], list):
                    # Handle the case where dataset_meta is a list
                    dataset_meta_list = checkpoint_meta['dataset_meta']
                    model.dataset_meta = {}
                    for dataset_meta in dataset_meta_list:
                        if 'classes' in dataset_meta:
                            model.dataset_meta['classes'] = dataset_meta['classes']
                        if 'palette' in dataset_meta:
                            model.dataset_meta['palette'] = dataset_meta['palette']
                else:
                    # mmdet 3.x, all keys should be lowercase
                    model.dataset_meta = {
                        k.lower(): v
